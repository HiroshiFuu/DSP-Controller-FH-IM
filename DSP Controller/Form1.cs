...
namespace DSP_Controller
{
    public partial class Form1 : Form
    {
        ...
        private byte[] ReqAddress = { 0x10, 0x20, 0x30, 0x40, 0x50, 0x01, 0x02, 0x03, 0x04, 0x05, 0x90, 0xA0}; //Rv1:10, Rv2:11
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            ReadDelegate = new ReadDelegateHandler(RxMessages);
            ProcessDelegate = new ProcessDelegateHandler(ProcessMessages);
            SendDelegate = new SendDelegateHandler(TxMessages);
            ...
            for (int i = 0; i < 200; i++)
            {
                ...
                chartLm.Series["lm"].Points.AddXY(i, 0);
                chartRs.Series["rs"].Points.AddXY(i, 0);
                chartLls.Series["lls"].Points.AddXY(i, 0);
                chartRr.Series["rr"].Points.AddXY(i, 0);
                chartLlr.Series["llr"].Points.AddXY(i, 0);
            ...
            setChartStyle(chartLm, "lm");
            setChartStyle(chartRs, "rs");
            setChartStyle(chartLls, "lls");
            setChartStyle(chartRr, "rr");
            setChartStyle(chartLlr, "llr");
            setChartStyle(chartRv1, "Rv1");
            setChartStyle(chartRv2, "Rv2");

            charts = new Chart[11];
            seriesNames = new string[11];
            charts[0] = chartSpeed;
            seriesNames[0] = "speed";
        ...
        private void setControls(bool state)
        {
            ...
            btnApplyTorque.Enabled = state;
            btnApplyTorqueCurve.Enabled = state;
        ...
        private void SendPara(byte MsgId)
        {
            serialMsgSend.MsgId = MsgId;
            serialMsgSend.setMsg();
            TxMessages();
        ...
        private void SendMessage(byte MsgId, float value)
        {
            serialMsgSend.MsgId = MsgId;
            serialMsgSend.MsgData = BitConverter.GetBytes(value);
            serialMsgSend.setMsg();
            TxMessages();
            if (ComPort.IsOpen)
                ComPort.Write(serialMsgSend.Message, 0, 8);
        ...
        private void SendMessage(byte MsgId, string value)
        {
            SendMessage(MsgId, float.Parse(value));
        }
        ...
        private int IncReq()
        {
            ...
            if (ReqIndex == number)
                ReqIndex = 0;
            if (!cbRv1Update.Checked && cbRv2Update.Checked && ReqIndex == 9)
                ReqIndex = 11;
            return ReqIndex++;
        }
        ...
        private void ResetChart(int index)
        {
            ...
            chart.Series["Series2"].Points[0].SetValueXY(0, 0);
            plot_index[index + 4] = 0;
        }
        #endregion
        ...
        private void timerCurve_Tick(object sender, EventArgs e)
        {
            ...
            if (step.time == timerCnt++)
            {
                ...
                {
                    timerCurve.Stop();
                    btnApplySpeedCurve.Enabled = btnApplyTorqueCurve.Enabled = true;
                    rtbSpeed.Enabled = rtbTorque.Enabled = true;
                    timerCnt = 1;
                    curveComm.Clear();
                }
            }
        ...
        private void rtbRev_TextChanged(object sender, EventArgs e)
        {
            if (rtbRev.Lines.Count() > 400)
                rtbRev.Text = "";
        }

        private void rtbSend_TextChanged(object sender, EventArgs e)
        {
            if (rtbSend.Lines.Count() > 400)
                rtbSend.Text = "";
        }

        private void rtbRev_DoubleClick(object sender, EventArgs e)
        {
            rtbRev.Text = "";
        }

        private void rtbSend_DoubleClick(object sender, EventArgs e)
        {
            rtbSend.Text = "";
        }
        ...
        {
            cbSpeed.Checked = cbTorque.Checked = cb_id.Checked = cb_iq.Checked = cbLm.Checked = cbRs.Checked = cbLls.Checked = cbRr.Checked = cbLlr.Checked = cbAuto.Checked;
            cbSpeed.Enabled = cbTorque.Enabled = cb_id.Enabled = cb_iq.Enabled = cbLm.Enabled = cbRs.Enabled = cbLls.Enabled = cbRr.Enabled = cbLlr.Enabled = !cbAuto.Checked;
            if (cbRv1Update.Checked)
        ...
        {
            if (btnApplyTorque.Enabled && e.KeyChar == 13)
                btnApplyTorque_Click(null, null);
        }
                ...
                for (; i <= RevMessagesOffset - 8; i++)
                {
                    if (RevMessages[i] == 0x1A || RevMessages[i] == 0x17 || RevMessages[i] == 0x1B)
                    {
                        ...
                        if (serialMsg[ReadMsgIndex].chkSumErr)
                        {
                            try
                            {
                                BeginInvoke(SetTextDelegate, "0x" + serialMsg[ReadMsgIndex].MsgId.ToString("X").PadLeft(2, '0') + " CRC Error\n");
                            }
                            catch { }
                            if (timeout.Enabled)
                                timeout.Stop();
                        ...
                        if (RevMessages[i] != 0x1A)
                        {
                            ...
                            rtbRev.SelectedText = string.Empty;
                            rtbRev.AppendText("Rev " + DisplayRx + "\n");
                            rtbRev.ScrollToCaret();
                        }
        ...
        private void ProcessMessages()
        {
            if (!serialMsg[LoadMsgIndex].chkSumErr)
                if (serialMsg[LoadMsgIndex].StartId == 0x1A)
                {
                    switch (serialMsg[LoadMsgIndex].MsgId)
                    {
                        case 0x10:
                            ...
                            chartSpeed.Series["speed"].Points[plot_index[4]].SetValueY(BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0));
                            chartSpeed.Series["Series2"].Points[0].SetValueXY(plot_index[4]++, BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0));
                        ...
                        case 0x20:
                            ...
                            chartTorque.Series["torque"].Points[plot_index[5]].SetValueY(BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0));
                            chartTorque.Series["Series2"].Points[0].SetValueXY(plot_index[5]++, BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0)); 
                        ...
                        case 0x30:
                            ...
                            chart_id.Series["id"].Points[plot_index[6]].SetValueY(BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0));
                            chart_id.Series["Series2"].Points[0].SetValueXY(plot_index[6]++, BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0));
                        ...
                        case 0x40:
                            ...
                            chart_iq.Series["iq"].Points[plot_index[7]].SetValueY(BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0));
                            chart_iq.Series["Series2"].Points[0].SetValueXY(plot_index[7]++, BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0));
                        ...
                        case 0x50:
                            ...
                            break;
                        case 0x90:
                            chartRv1.Series["Rv1"].Points[plot_index[13]].SetValueY(BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0));
                            chartRv1.Series["Series2"].Points[0].SetValueXY(plot_index[13]++, BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0));
                            chartRv1.ChartAreas[0].RecalculateAxesScale();
                            if (plot_index[13] == 200)
                            {
                                plot_index[13] = 0;
                            ...
                            break;
                        case 0xA0:
                            chartRv2.Series["Rv2"].Points[plot_index[14]].SetValueY(BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0));
                            chartRv2.Series["Series2"].Points[0].SetValueXY(plot_index[14]++, BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0));
                            chartRv2.ChartAreas[0].RecalculateAxesScale();
                            if (plot_index[14] == 200)
                            {
                                plot_index[14] = 0;
                            ...
                            break;
                        case 0x01:
                            tbLm.Text = BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0).ToString();
                            chartLm.Series["lm"].Points[plot_index[8]].SetValueY(BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0));
                            chartLm.Series["Series2"].Points[0].SetValueXY(plot_index[8]++, BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0));
                            chartLm.ChartAreas[0].RecalculateAxesScale();
                            if (plot_index[8] == 200)
                            {
                                plot_index[8] = 0;
                                if (cbLm.Checked)
                                    WriteToFile(chartLm, "lm");
                            }
                            break;
                        case 0x02:
                            tbRs.Text = BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0).ToString();
                            chartRs.Series["rs"].Points[plot_index[9]].SetValueY(BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0));
                            chartRs.Series["Series2"].Points[0].SetValueXY(plot_index[9]++, BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0));
                            ...
                            break;
                        case 0x03:
                            tbLls.Text = BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0).ToString();
                            chartLls.Series["lls"].Points[plot_index[10]].SetValueY(BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0));
                            chartLls.Series["Series2"].Points[0].SetValueXY(plot_index[10]++, BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0));
                            chartLls.ChartAreas[0].RecalculateAxesScale();
                            if (plot_index[10] == 200)
                            {
                                plot_index[10] = 0;
                                if (cbLls.Checked)
                                    WriteToFile(chartLls, "lls");
                            }
                            break;
                        case 0x04:
                            tbRr.Text = BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0).ToString();
                            chartRr.Series["rr"].Points[plot_index[11]].SetValueY(BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0));
                            chartRr.Series["Series2"].Points[0].SetValueXY(plot_index[11]++, BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0));
                            chartRr.ChartAreas[0].RecalculateAxesScale();
                            if (plot_index[11] == 200)
                            {
                                plot_index[11] = 0;
                                if (cbRr.Checked)
                                    WriteToFile(chartRr, "rr");
                            }
                            break;
                        case 0x05:
                            tbLlr.Text = BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0).ToString();
                            chartLlr.Series["llr"].Points[plot_index[12]].SetValueY(BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0));
                            chartLlr.Series["Series2"].Points[0].SetValueXY(plot_index[12]++, BitConverter.ToSingle(serialMsg[LoadMsgIndex].MsgData, 0));
                            chartLlr.ChartAreas[0].RecalculateAxesScale();
                            if (plot_index[12] == 200)
                            {
                                plot_index[12] = 0;
                                if (cbLlr.Checked)
                                    WriteToFile(chartLlr, "llr");
                            }
                            break;
                    }
                }
                else if (serialMsg[LoadMsgIndex].StartId == 0x1B)
                {
                    ...
                    //        break;
                    //    default:
                    //        int index = Array.IndexOf(ParaAddress, serialMsg.MsgId);
        ...
        private void TxMessages()
        {
            string DisplayTx = "Tx ";
            DisplayTx += BitConverter.ToString(serialMsgSend.Message);
            rtbSend.SelectedText = string.Empty;
            rtbSend.AppendText(DisplayTx + "\n");
        ...
        private void btnSend_Click(object sender, EventArgs e)
        {
            ...
            //if (ComPort.IsOpen)
            //    ComPort.Write(serialMsg.Message, 0, 8);
            //this.Invoke(SendDelegate);
            //serialMsg.Message[0] = Convert.ToByte(tbCommand.Text.Substring(0, 2), 16);
            //serialMsg.Message[1] = Convert.ToByte(tbCommandId.Text.Substring(0, 2), 16);
            //serialMsg.Message[5] = Convert.ToByte(tbCommandData.Text.Substring(0, 2), 16);
            //serialMsg.Message[4] = Convert.ToByte(tbCommandData.Text.Substring(2, 2), 16);
            //serialMsg.Message[3] = Convert.ToByte(tbCommandData.Text.Substring(4, 2), 16);
            //serialMsg.Message[2] = Convert.ToByte(tbCommandData.Text.Substring(6, 2), 16);
            //serialMsg.Message[6] = serialMsgSend.MsgChksum[0];
            //serialMsg.Message[7] = serialMsgSend.MsgChksum[1];
            //serialMsg.getMsg();
            //this.Invoke(ReadDelegate);
            //MessageBox.Show(serialMsg.chkSumErr.ToString());
        }
        ...
        private void btnApplySpeedCurve_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                float.Parse(tb_id2.Text);
            }
            catch
            {
                string value = tb_id.Text.Trim();
                if (value != "" && value != "-")
                    errorProvider2.SetError(tb_id2, "Invalid Value");
                return;
            }
            btnApplySpeedCurve.Enabled = btnApplyTorqueCurve.Enabled = false;
        ...
        private void btnApplyTorqueCurve_Click(object sender, EventArgs e)
        {
            if (!validateCurveEntry(rtbTorque))
            {
                errorProvider2.Clear();
                errorProvider2.SetError(lbTorqueCurve, "Error Entry");
                curveComm.Clear();
                stepCnt = 0;
                timerCnt = 1;
                rtbTorque.SelectAll();
                rtbTorque.ScrollToCaret();
                rtbTorque.Focus();
                return;
            }
            try
